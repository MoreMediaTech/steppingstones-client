...
const BusinessInsuranceSection = ({ id }: { id: string }) => {
  ...
  const [updateOrCreateBusinessInsurance, { isLoading }] = useUpdateOrCreateBusinessInsuranceMutation()

  const [value, setValue] = useState(countyData?.supportForStartups?.businessInsurance?.content)
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.supportForStartups?.businessInsurance?.title,
    },
  })
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        await updateOrCreateBusinessInsurance(formData).unwrap()
        reset()
        setIsEdit(false)
        router.replace({
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.supportForStartups?.businessInsurance ? (
              <ContentPreview content={countyData?.supportForStartups?.businessInsurance} />
            ) : (
              <CountySectionForm
                register={register}
...
export default BusinessInsuranceSection
