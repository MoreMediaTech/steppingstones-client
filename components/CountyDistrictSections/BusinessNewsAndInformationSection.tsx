...
const BusinessNewsAndInformationSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(countyData?.businessNewsAndInformation?.content)
  ...
  const {
    ...
    useEffect(() => {
      ...
      reset({ title: countyData?.businessNewsAndInformation?.title })
    }, [countyData])
  ...
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        showNotification({
          message: 'Error saving Business News and Information content',
          autoClose: 3000,
          color: 'red'
        })
      }
