...
const ImproveSkillsSection = ({ id }: { id: string }) => {
 ...
 const [updateOrCreateImproveSKills, { isLoading }] = useUpdateOrCreateImproveSkillsMutation()
 ...
 const {
  ...
  useEffect(() => {
    ...
    setValue(countyData?.growingABusiness?.improveSkills?.content)
  }, [countyData])
 ...
 const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> = useCallback(
   async (data) => {
     try {
       ...
       await updateOrCreateImproveSKills(formData).unwrap()
       refetchCounty()
        setIsEdit(false)
     } catch (error) {
       dispatch(setError({ message: error.message }))
       showNotification({
        message: 'Error updating improve skills content',
        autoClose: 3000,
        color: 'red'
       })
     }
