...
const DevelopProductsAndServicesSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(
    countyData?.growingABusiness?.developProductsAndServices?.content
  )
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.growingABusiness?.developProductsAndServices?.title,
    },
  })

    useEffect(() => {
      // reset the form when the county data is changed/updated
      reset({ title: countyData?.growingABusiness?.developProductsAndServices?.title })
      setValue(countyData?.growingABusiness?.developProductsAndServices?.content)
    }, [countyData])
  ...
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        showNotification({
          message:  'Error updating Develop Products and Services content',
          autoClose: 3000,
          color: 'red',
        })
      }
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.growingABusiness?.developProductsAndServices ? (
              <ContentPreview
                content={countyData?.growingABusiness?.developProductsAndServices}
              />
            ) : (
              <CountySectionForm
                register={register}
...
export default DevelopProductsAndServicesSection
