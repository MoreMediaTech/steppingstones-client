...
const BusinessPlansSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(countyData?.supportForStartups?.businessPlans?.content)
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.supportForStartups?.businessPlans?.title,
    },
  })
  ...
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        showNotification({
          message: 'Error updating business plans content',
          autoClose: 3000,
          color: 'red'
        })
      }
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.supportForStartups?.businessPlans ? (
              <ContentPreview content={countyData?.supportForStartups?.businessPlans} />
            ) : (
              <CountySectionForm
                register={register}
