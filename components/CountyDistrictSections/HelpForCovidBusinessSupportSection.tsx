...
const HelpForCovidBusinessSupportSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(countyData?.topicalBusinessIssues?.helpForCovidBusinessSupport?.content)
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.topicalBusinessIssues?.helpForCovidBusinessSupport?.title,
    },
  })

  useEffect(() => {
    // reset the form when the county data is changed/updated
    reset({ title: countyData?.topicalBusinessIssues?.helpForCovidBusinessSupport?.title })
  }, [countyData])
  ...
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        showNotification({
          message: 'Error updating topical business issues content',
          autoClose: 3000,
          color: 'red',
        })
      }
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.topicalBusinessIssues?.helpForCovidBusinessSupport ? (
              <ContentPreview content={countyData?.topicalBusinessIssues?.helpForCovidBusinessSupport} />
            ) : (
              <CountySectionForm
                register={register}
...
export default HelpForCovidBusinessSupportSection
