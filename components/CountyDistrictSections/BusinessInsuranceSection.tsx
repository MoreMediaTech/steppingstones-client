...
const BusinessInsuranceSection = ({ id }: { id: string }) => {
  ...
  const [updateOrCreateBusinessInsurance, { isLoading }] =
    useUpdateOrCreateBusinessInsuranceMutation()

  const [value, setValue] = useState<string>(
    countyData?.supportForStartups?.businessInsurance?.content
  )
    ...
    useCallback(
      async (data) => {
        try {
          ...
          showNotification({
            message: 'Error updating County Business Insurance content',
            autoClose: 3000,
            color: 'red',
          })
        }
        ...
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.supportForStartups?.businessInsurance ? (
              <ContentPreview
                content={countyData?.supportForStartups?.businessInsurance}
              />
            ) : (
              <CountySectionForm
                register={register}
