...
const BusinessParksSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(
    districtData?.businessParks?.content
  )
  ...
  const {
    ...
    defaultValues: {
      title: districtData?.businessParks?.title,
    },
  })

  useEffect(() => {
    // reset the form when the county data is changed/updated
    reset({ title: districtData?.businessParks?.title })
    setValue(districtData?.businessParks?.content)
  }, [districtData])
  ...
  const submitHandler: SubmitHandler<EditorFormDataProps> = useCallback(
    async (data) => {
      try {
        ...
        showNotification({
          message: 'Error updating district business parks content',
          autoClose: 3000,
          color: 'red',
        })
      }
      ...
      <section className="container">
        {isLoadingDistrict ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && districtData?.businessParks ? (
              <ContentPreview content={districtData?.businessParks} />
            ) : (
...
export default BusinessParksSection
