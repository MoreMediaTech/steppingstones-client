...
const SupportForEstablishedBusinessSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(
    countyData?.supportForEstablishedBusiness?.content
  )
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.supportForEstablishedBusiness?.title,
    },
  })

  useEffect(() => {
    // reset the form when the county data is changed/updated
    reset({ title: countyData?.supportForEstablishedBusiness?.title })
    setValue(countyData?.supportForEstablishedBusiness?.content)
  }, [countyData])
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.supportForEstablishedBusiness ? (
              <ContentPreview content={countyData?.supportForEstablishedBusiness} />
            ) : (
              <CountySectionForm
                register={register}
...
export default SupportForEstablishedBusinessSection
