...
const HelpForHeritageAndTourismSection = ({ id }: { id: string }) => {
  ...
  const [updateOrCreateHeritageAndTourism, { isLoading }] = useUpdateOrCreateHeritageAndTourismMutation()

  const [value, setValue] = useState<string>(countyData?.topicalBusinessIssues?.helpForHeritageAndTourism?.content)
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.topicalBusinessIssues?.helpForHeritageAndTourism?.title,
    },
  })
  ...
  const submitHandler: SubmitHandler<Partial<EditorFormDataProps>> =
    useCallback(async (data) => {
      try {
        ...
        await updateOrCreateHeritageAndTourism(formData).unwrap()
        refetchCounty()
        setIsEdit(false)
      } catch (error) {
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.topicalBusinessIssues?.helpForHeritageAndTourism ? (
              <ContentPreview content={countyData?.topicalBusinessIssues?.helpForHeritageAndTourism} />
            ) : (
              <CountySectionForm
                register={register}
...
export default HelpForHeritageAndTourismSection
