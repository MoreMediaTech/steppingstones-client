...
const MarketResearchSection = ({ id }: { id: string }) => {
  ...
  const [value, setValue] = useState<string>(countyData?.supportForStartups?.marketResearch?.content)
  ...
  const {
    ...
    defaultValues: {
      title: countyData?.supportForStartups?.marketResearch?.title,
    },
  })
    ...
    useCallback(async (data) => {
      try {
        const formData = {
          title: data.title,
          content: value,
          countyId: countyData?.supportForStartups?.marketResearch?.id,
      ...
      <section className="container">
        {isLoadingCounty ? (
          ...
          <Paper shadow="lg" p="md" radius="md" withBorder className="w-full">
            ...
            {!isEdit && countyData?.supportForStartups?.marketResearch ? (
              <ContentPreview content={countyData?.supportForStartups?.marketResearch} />
            ) : (
              <CountySectionForm
                register={register}
