    ...
    <section className="relative md:px-20 mx-auto overflow-auto max-w-screen-xl md:py-8 w-full">
      <div className="grid w-full grid-cols-1 gap-4 overflow-auto  py-4 md:grid-cols-3 md:gap-8 ">
        {links.map((link, index) => (
          <div key={`${link}-${index}`}>
            <Link href={`/admin/editor-portal/${link.path}`} className="mx-4 flex h-40 flex-col items-center justify-center rounded-xl bg-[#5E17EB] text-lg font-semibold text-white shadow-lg transition delay-150 duration-300 
