...
export default function EditSection({ sectionId }: EditSectionProps) {
  ...
  const {
    ...
    updateSectionById,
    refetchSection,
  } = useSectionController(sectionId, undefined, undefined);
  ...
  return (
    <section className="w-full">
      <div className="flex items-center justify-between w-full px-1 py-4 sm:px-2">
        <Header title={`Edit ${section?.name} Section`} order={2} />
        <Button
          variant="outline"
          className="w-1/3"
          onClick={() => router.back()}
