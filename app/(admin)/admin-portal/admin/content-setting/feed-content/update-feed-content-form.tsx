...
type Props = {
  ...
  feedContent: FeedContentSchemaProps | null;
};

export function UpdateFeedContentForm({ feedContent, buttonTitle }: Props) {
  const [open, setOpen] = React.useState(false);
  const defaultValues = {
    name: feedContent?.name ? (feedContent?.name as string) : "",
    published: feedContent?.published
      ? (feedContent?.published as boolean)
      : false,
  };
  const { form, updateHandler, preview, onChangePicture } =
    useFeedContentSettingController(defaultValues, feedContent?.id, setOpen);
