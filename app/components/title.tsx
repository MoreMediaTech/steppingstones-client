...
function extractValueByKey <T extends object>(array: readonly T[], keyToFind: keyof T ): T[keyof T] | undefined {
...
export default function Title ({ order, lineClamp, className, children}: TitleProps){
  ...
  if(element[order] === 'h1'){
    return (
        <h1
          className={cn( `font-bold ${fontSize} ${lineHeight} ${truncate}`, className)}
        >
          {children}
        </h1>
      );
